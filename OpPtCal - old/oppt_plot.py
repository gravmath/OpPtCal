...
#  Author:   Unknown
#
#  Purpose:  Produces plots and fits for OpPtCal
#
#  Language: Python 2.7
#
#  Packages: numpy
...
#            opptcal_paths
#
#  History:  1) Initial version (unknown) 3/27/2015
...
def plot_tswpsum(fig,axes,cs,eii,trs,ms,aaxt,unit,head,cii,output_dir,oppt_dir):
    ...
    oppt_param = np.zeros((len(cs),2,16,8,2),np.float)
    ...
    if eii > 0:
        ...
        #Sum over all energies and then all iterations for a given unit (cii), head (head).
        #Results in a 2D array with pixel and cumulative sweep as indices 0 and 1
    ...
    if Nmax > 0:                
        ...
        for px in range(16):
            ...
            if(len(np.where(Nd00 > 0)[0]) > 3):
                ...
                # skip 1st and last point... DJG 6/20/2016
                p1[0] = 0.
                p3[-1] = 0.
                ...
                for ii in range(3):
                    ...
                    # skip 1st and last point... DJG 6/20/2016
                    # skip 1st point and include the last point... CS 11/10/2016
                    cNd00[0] = 0.
                    #cNd00[-1] = 0.
                    ...
                    guesses = np.array([lim0cr,Q,aaxt[0]]) #,gam])

                    l0s[px,ii,0] = lim0cr
                    
                    if lim0cr > 0 and Q > 0 and gam > 0 and len(np.where(cNd00 > 0)[0]) > 3:
                       
                        kd,cov,infodict,mesg,ier = optimize.leastsq(of.N_fitres,guesses,args=(trs[cNd00>0],cNd00[cNd00>0],px,aaxt,gam,unit),full_output=True)
                        ...
                        (lim0cr,Q,aaxtprime0) = kd#gamma) = kd
                        (lim0crerr,Qerr,aaxterr0) = kderr#,gammaerr) = kderr
                        gamma = gam
                        gammaerr = 0.
                        
                        caaxt = [aaxtprime0,aaxtprime0/10.]
                        
                        
                        if lim0cr > 0:
                            (N,Nxt1,Nxt2) = of.Ns(trsf,lim0cr,Q,gamma,caaxt)
                            (Nt,Nxt1t,Nxt2t) = of.Ns(trs,lim0cr,Q,gamma,caaxt)
                            ...
                            if len(R2s[R2s > 0]) > 0:
                                if ii == 0 or (ii == 1 and R2[px,ii]<minr2s[px] and np.abs(lratio-1.) < 0.3 ) or (ii == 2 and R2[px,ii]<minr2s[px] and np.abs(lratio-1.) < 0.3 and len(np.where(cNd00 > 0)[0]) > 8):
                                    ...
                                    # added best R2 of fit and total counts
                                    oppt_param[cii,head,px,5,0] = R2[px,ii]
                                    oppt_param[cii,head,px,6,0] = np.sum(Nd00[px])
                                    oppt_param[cii,head,px,7,0] = caaxt[0]
...
def plot_phdparam(fig,axes,oppt_param,unit,serial,oppt_dir,specfit_dir,output_dir):
    ...
    for head in range(2):
        ...
        fname = '%s/%s/%s%sH%i_%s.csv' % (paths.OpPt_specfit_path,specfit_dir,sensor.upper(),str(unit).zfill(3),head,specfit_dir)
