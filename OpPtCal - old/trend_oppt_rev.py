...
#  $LastChangedDate: 2016-02-16 17:36:45 -0500 (Tue, 16 Feb 2016) $
...
for obs in [1,2,3,4]:
    for unit in np.hstack( (np.arange(1,17),np.arange(201,217))):
        for head in range(2):
            ...
            data_path = '/data/ftp/mms%i/fpi/cal/OpPtCal' % obs
      ...
      #              axes[2].plot(orbs,np.polyval(ps,orbs),'r--')
       ...
       #             axes[2].plot(orbs,np.polyval(ps,orbs),'r--')                    
                    ...
                    if unit > 200:
                        ...
                        sensor = 'DIS'
                    ofname = '/fpishare/Gershman/OpPtCal/output_newrepository_rev/%s%s_trend' % (sensor,prefix)
